...
echo '"quota" : {' >>/usr/lib/cgi-bin/tmp/test3
...
curl -H "Content-Type: application/json"  --data @/usr/lib/cgi-bin/tmp/test3 -s -X PUT -H "X-Auth-Token: $TOKEN" https://$IP:9696/v2.0/quotas/$PRJID -k  |python -m json.tool > /usr/lib/cgi-bin/tmp/updated-neutron-qouta

echo "<h1><font color="#FFC300"> Updated Network Quota for Project  $PRJNAME </font></h1>"
echo " <table border="2" style="width:80%">
  <tr>
    <th><font color="#FFC300">Resource-Name</font></th>
    <th><font color="#FFC300">Values</font></th>
  </tr>"
   name=`cat /usr/lib/cgi-bin/tmp/updated-neutron-qouta | grep -v "quota_set" | grep -v id | grep -v { | grep -v } | awk {'print $1'}`
   for i in $name;
      do
        echo "<tr><td><font color="#FFFFFF">`grep $i /usr/lib/cgi-bin/tmp/updated-neutron-qouta | awk {'print $1'} | sed -e 's/://g' | sed -e 's/"//g'` </font></td>"
        echo "<td><font color="#FFFFFF">`grep $i /usr/lib/cgi-bin/tmp/updated-neutron-qouta | awk {'print $2'} | sed -e 's/://g' | sed -e 's/,//g'` </font></td></tr>"
      done
echo "</table>"
echo "</form>"
echo "</center>"
